...
| 脚本工具 | `scripts/*.py`（仓库根） | 13 个，显式调用；其中 3 个 per-需求脚本仅溯源用，新用例一律走 `gen_xmind.py` |
...
| `docs/lessons-learned.md` | 历史台账，**不会自动加载** | 里面写的是当时的旧路径（`.trae/scripts/` 等），作为台账没错，但别当现状 |
...
用 `AskUserQuestion` 问，**默认推荐 XMind 场景树**。全部产出物的格式与落位以 `output-schema.md` 矩阵为准。
...
### 3. 记忆只记跨需求工程经验，业务口径落台账

跨需求可复用的工程经验（踩坑、降级处理、被纠正的理解）→ `UpdateMemory` + 同步 `known-pitfalls.md`；
单需求业务口径 → XMind 标注 + 台账，**不落记忆**。
Qoder 记忆是本机文件不随 git 同步，团队要用的信息必须进仓库内资产。详见 `memory-discipline.md`。
...
本项目为 QA 团队共用，通过 git 协作：

- 新成员上手四步见 `README.md`（填凭据 → 重启 IDE → 验 hook → 跑冒烟）
- **`.qoder/` 下的任何修改 = 修改全员的 agent 行为**，走 PR review；改过 hooks/settings 的注明「需重启 IDE」
- Qoder 本机记忆不随 git 同步，团队要用的口径/经验必须落仓库内资产（rules / docs）
...
| 约束 | 兜底手段 | 行为 | 验证状态 |
|---|---|---|---|
| 禁止在 `docs/test-cases/` 下写 `.md` 交付物 | `PreToolUse` + `deny` hook | **直接写不进去** | ✅ 已实测（指纹 B9K2）；⚠️ settings.json 改环境变量路径后待重启复验 |
| 真实工具失败（fetch 拿 401/403、超时、MCP 崩） | `PostToolUseFailure` hook | 注入处置协议 | ⚠️ 未实测 |
| shell 命令非零退出 / 空数据 / 格式不符 | `failure-protocol.md` 规则 | 模型按协议处置 | ✅ 已实测生效 |

（shell 非零退出不算「工具失败」——走 `PostToolUse` 不是 `PostToolUseFailure`，详见 `known-pitfalls.md` 第 3 节。）
