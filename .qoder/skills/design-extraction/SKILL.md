...
description: "Use when extracting test points from visual inputs (蓝湖/墨刀/Confluence/飞书/截图). Triggers on 设计稿、原型稿、截图识别、UI 提取 etc."
---
...
- 需要从视觉类输入（蓝湖/墨刀/Confluence/飞书/截图）提取测试关注点
- 触发词：设计稿、原型稿、截图识别、UI 提取、蓝湖、墨刀

## 核心问题

LLM agent 无法直接"看到"图片内容。本 skill 把不可见的视觉内容转化为可分析的结构化文本。
...
python3 scripts/extract_confluence_images.py <pageId> --output /tmp/confluence_images/
```
...
- 配置 `.mcp.json` 引用蓝湖 MCP（Qoder 的 MCP 配置在仓库根 `.mcp.json`，不是 `.trae/mcp.json`）
...
墨刀是 SPA，直接 curl 拿到的是空壳，无法获取原型内容

方案1：导出 HTML（推荐）
- 在墨刀项目 → 浏览器「另存为网页（完整 HTML）」
- 把 HTML 文件放到 /tmp/modao_prototype.html
- agent 用 HTMLParser 解析 DOM 提取可见文本和结构化信息
...
python3 scripts/ocr_images.py /tmp/design_images/ --lang chi_sim --output /tmp/ocr_result.json
```
OCR 能提取：文案、按钮文字、标签
...
- 墨刀/蓝湖等 SPA 设计稿工具，**直接 curl 拿不到内容**，让用户用浏览器「另存为网页（完整 HTML）」保存
- 技术方案与原型/PRD 的数值/口径不一致时，必须当作待确认问题，不能默认以技术方案为准
- GLM-5.2 不支持视觉识别，遇到图片必须走 OCR 或提示用户切换视觉模型
